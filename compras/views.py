...
def acquisition(request, acquisition_id=None):
    ...
    if request.method == 'POST':
        ...
        if not acquisition_id:
            ...
            header = AcquisitionHeader.objects.filter(
                pk=acquisition_id).first()
            if header:
        ...
        if detail:
            detail.save()
            subtotal = AcquisitionDetail.objects.filter(
                acquisition=acquisition_id).aggregate(Sum('subtotal'))
            discount = AcquisitionDetail.objects.filter(
                acquisition=acquisition_id).aggregate(Sum('discount'))
